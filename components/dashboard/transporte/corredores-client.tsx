  ...
  return (
    <div className="max-w-5xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {corridors.map((c) => (
            <div key={c.id} className="bg-card rounded-3xl p-5 card-elevated border border-border/60 hover:-translate-y-0.5 transition-all">
              ...
              <h4 className="mt-4 font-bold text-lg leading-tight">{c.name}</h4>
              <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
                <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4 text-emerald-500" />{c.partida_ciudad || c.partida_aduana || "—"}</span>
                <span className="text-muted-foreground/50">→</span>
                <span className="inline-flex items-center gap-1"><Flag className="h-4 w-4 text-blue-500" />{c.destino_ciudad || c.destino_aduana || "—"}</span>
              </div>
              <div className="mt-3 pt-3 border-t border-border/60 flex items-center gap-1.5 flex-wrap">
                ...
                {Array.isArray(c.paises_paso) && c.paises_paso.length > 0 && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-violet-50 text-violet-600">{c.paises_paso.length} país(es) de paso</span>
                )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl p-0 border-0 rounded-3xl">
          ...
          <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
            ...
            {/* Recorrido: paradas lado a lado (ancho, no torre) */}
            <div>
              <h4 className="text-sm font-bold mb-3 flex items-center gap-2"><Route className="h-4 w-4" /> Recorrido</h4>
              <div className="grid sm:grid-cols-2 gap-3">
                <Stop color="bg-emerald-100 text-emerald-600" icon={<MapPin className="h-4 w-4" />} title="1 · Partida">
                ...
                <Stop color="bg-amber-100 text-amber-600" icon={<LogOut className="h-4 w-4" />} title="2 · Salida (frontera)">
                  ...
                  <Field label="Lugar operativo" name="salida_lugar_operativo" def={editing?.salida_lugar_operativo} small className="col-span-2" />
                </Stop>

                <Stop color="bg-blue-100 text-blue-600" icon={<LogIn className="h-4 w-4" />} title="3 · Entrada">
                  ...
                  <Field label="Lugar operativo" name="entrada_lugar_operativo" def={editing?.entrada_lugar_operativo} small className="col-span-2" />
                </Stop>

                <Stop color="bg-[#1C1C28] text-[#D1F366]" icon={<Flag className="h-4 w-4" />} title="4 · Destino">
              ...
              <div className="mt-3 rounded-2xl bg-muted/40 p-4">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground flex items-center gap-1.5">
                    <Milestone className="h-3.5 w-3.5 text-violet-500" /> Países de paso (entre salida y entrada)
                  </p>
                ...
                {pasos.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {pasos.map((p, i) => (
                      <div key={i} className="flex items-center gap-2 rounded-xl bg-card border border-border p-2">
            ...
            <div className="rounded-2xl bg-muted/40 p-4">
              <h4 className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-3 flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> Plazos por defecto</h4>
...
function Stop({ color, icon, title, children }: { color: string; icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-muted/40 p-4">
      <div className="flex items-center gap-2 mb-3">
        <span className={`h-8 w-8 rounded-xl grid place-items-center ${color}`}>{icon}</span>
...
function Field({ label, name, def, type = "text", placeholder, small, className }: { label: string; name: string; def?: string | number | null; type?: string; placeholder?: string; small?: boolean; className?: string }) {
  return (
    <div className={`space-y-1.5 ${className || ""}`}>
      <Label htmlFor={name} className="text-xs text-muted-foreground">{label}</Label>
      <Input id={name} name={name} type={type} defaultValue={def ?? undefined} placeholder={placeholder} className={`rounded-xl bg-card ${small ? "h-9" : "h-10"}`} />
    </div>
  )
}
