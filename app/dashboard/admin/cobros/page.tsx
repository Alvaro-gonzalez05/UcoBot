...
import { CobrosTabs } from "@/components/dashboard/admin/cobros-tabs"

/**
 * Cobros: todo lo que entra, en un solo lugar.
 *
 * Reemplaza a /dashboard/admin/payments (Mercado Pago) y /dashboard/admin/cobranza
 * (facturación manual), que respondían la misma pregunta desde dos pantallas
 * distintas. Los números de arriba son de la plataforma entera; las pestañas
 * separan por cómo cobra cada cliente, que es la única diferencia real.
 */
...
function statusLabel(u: any): { text: string; cls: string } {
  if (u.billing_exempt)
    return { text: "Pago manual", cls: "bg-emerald-100 text-emerald-800 border-emerald-200" }
  ...
  if (u.subscription_status === "past_due")
    return { text: "Pago vencido", cls: "bg-red-100 text-red-800 border-red-200" }
  if (u.subscription_status === "cancelled")
    return { text: "Cancelada", cls: "bg-neutral-100 text-neutral-700 border-neutral-200" }
...
export default async function AdminCobrosPage() {
  ...
  // TODOS los usuarios, incluidos los de pago manual que no tienen cobros de MP.
  const perUser = (users || [])
    .map((u) => {
      ...
      return {
        ...
        count: v.count,
        last: v.last,
        name: u.business_name || "Usuario",
      }
    })
    .sort((a, b) => b.total - a.total)

  const stats = [
    { label: "Recaudado total", value: fmt.format(totalAllTime), icon: DollarSign, cls: "bg-green-50 dark:bg-green-900/30 text-green-500" },
    { label: "Este mes", value: fmt.format(totalThisMonth), icon: TrendingUp, cls: "bg-[#D1F366]/15 text-[#4a7c00] dark:text-[#D1F366]" },
    { label: "Suscriptores activos", value: String(activeSubscribers), icon: Users, cls: "bg-blue-50 dark:bg-blue-900/30 text-blue-500" },
    { label: "Cobros totales", value: String(allPayments.length), icon: Receipt, cls: "bg-violet-50 dark:bg-violet-900/30 text-violet-500" },
  ]
    ...
    <div className="bg-card rounded-3xl shadow-sm border border-border overflow-hidden">
      <div className="px-6 pt-5 pb-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Estado de la suscripción, total recaudado y pago manual.
        </p>
      </div>
          ...
          perUser.map((u) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {u.count > 0
                        ? `${u.count} cobro${u.count !== 1 ? "s" : ""}${
                            u.last ? ` · último ${new Date(u.last).toLocaleDateString("es-AR")}` : ""
                          }`
                ...
                <span
                  className={`hidden sm:inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold border ${st.cls}`}
                >
                  {st.text}
                </span>
                <span className="text-sm font-bold text-green-600 dark:text-green-400 w-20 text-right flex-shrink-0">
                  {fmt.format(u.total)}
                </span>
  ...
  const movimientos = (
    <div className="bg-card rounded-3xl shadow-sm border border-border overflow-hidden">
      <div className="px-6 pt-5 pb-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Pagos aprobados de Mercado Pago, del más reciente al más viejo.
        </p>
      </div>
      <div className="divide-y divide-border">
        {allPayments.length > 0 ? (
            ...
            <div key={p.id} className="flex items-center gap-4 px-6 py-3.5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate dark:text-white">
                  {userById.get(p.user_id)?.business_name || "Usuario"}
                </p>
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-2 text-muted-foreground">
            ...
            <p className="text-sm">Todavía no hay cobros registrados.</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4 px-1 pt-2">
        ...
        <div>
          <h2 className="text-3xl font-bold dark:text-white">Cobros</h2>
          <p className="text-muted-foreground text-sm mt-0.5">
            Suscripciones de Mercado Pago y facturación manual, todo junto.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          ...
          >
            <div
              className={`w-12 h-12 ${s.cls} rounded-2xl flex items-center justify-center flex-shrink-0`}
            >
            ...
            <div className="min-w-0">
              ...
              <p className="text-2xl font-bold dark:text-white truncate">{s.value}</p>
            </div>
          </div>
        ))}
      </div>

      <CobrosTabs clientes={clientes} movimientos={movimientos} manual={<BillingManager />} />
    </div>
  )
}
